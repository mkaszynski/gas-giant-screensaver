...
float ringSunTransmissionFiltered(vec3 point,vec3 sun,float pixelFootprint){
 ...
 float lod=max(0.,log2(max(pixelFootprint,footprint*.35)*4096./(uRingBounds.y-uRingBounds.x)));
...
float ringSunTransmission(vec3 point,vec3 sun){
 float mu=dot(uRingNormal,sun);
 float t=-dot(point-uRingCenter,uRingNormal)/(abs(mu)<1.e-7?1.e-7:mu);
 float radius=length(point+sun*t-uRingCenter);
 return ringSunTransmissionFiltered(point,sun,fwidth(radius));
}
